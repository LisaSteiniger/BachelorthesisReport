\chapter{Theorie und Berechnung der Erosion}\label{chap:theory}
...
\begin{table}[!]
	\caption{Werte für den Parameter \(E_{TF}\) in (\unit{eV}) zur Berechnung der physikalischen Zerstäubungsausbeute \cite{PWI-Dirk}}\label{tab:ETF}
	\centering
	\begin{tabular}{|c||c|c|c|c|c|c|}\hline
		Target/Ion&Wasserstoff&Deuterium&Tritium&Helium&Selbstzerstäubung&Sauerstoff\\\hline\hline
...
\begin{table}[!]
	\caption{Werte für die Schwellenenergie der physikalischen Zerstäubung \(E_{th}\) in (\unit{eV}) zur Berechnung der physikalischen Zerstäubungsausbeute \cite{PWI-Dirk}}\label{tab:EthPhys}
	\centering
	\begin{tabular}{|c||c|c|c|c|c|c|}\hline
		Target/Ion&Wasserstoff&Deuterium&Tritium&Helium&Selbstzerstäubung&Sauerstoff\\\hline\hline
...
\begin{table}[!]
	\caption{Werte für die Sublimationswärme \(E_s\) in (\unit{eV}) zur Berechnung der physikalischen Zerstäubungsausbeute \cite{PWI-Dirk}}\label{tab:Es}
	\centering
...
\begin{table}[!]
	\caption{Werte für die Fitparameter für die Fitfunktionen \(f\), \(b\) und \(c\) zur Berechnung der physikalischen Zerstäubungsausbeute mit\\\(Fitfunktion = y0 + \sum A_i \cdot exp\left(-\frac{E - x0}{t_i}\right)\) \cite{BehrischEckstein}}\label{tab:fitMarkus1}
	\centering
...
\begin{table}[!]
	\caption{Werte für den Fitparameter \(Y0\) zur Berechnung der physikalischen Zerstäubungsausbeute in Abhängigkeit der Teilchenenergie \(E\) in (\unit{eV}) mit \(E_{min} \ll E \le E_{max}\) \cite{BehrischEckstein}}\label{tab:fitMarkus2}
	\centering
...
Chemische Erosion basiert auf der chemischen Reaktion mit den auftreffenden Ionen. Im Fall von Sauerstoff bildet sich das flüchtige Kohlenstoffmonoxid und in deutlich geringeren Mengen auch Kohlenstoffdioxid
% (Quelle?)
.\\
...
Zerstäubungsprozesse zu berechnen ist eine keineswegs triviale Aufgabe. Die dazu notwendigen Formeln auf analytische Art herzuleiten ist in den seltensten Fällen möglich, da dazu die integralen Transportgleichungen gelöst werden müssen. Sigmund
%(Quelle) 
hat sich ausführlich damit beschäftigt und für Atom-Atom-Kollisionen unter Vernachlässigung der Energieverluste an Elektronen eine gute Näherung gefunden, die jedoch nur für spezielle Potentiale Gültigkeit besitzt. In der Praxis wird wegen dieser Schwierigkeiten statt eines analytischen Ansatzes in der Regel eher das ingenieurswissenschaftliche Vorgehen gewählt: Erhobene Datensets werden ausgewertet und durch semi-empirische Fits beschrieben. Auf dieser Grundlage haben sich mehrere konkurrierende Gleichungen ergeben, die der Ermittlung von Zerstäubungsausbeuten dienen. Entsprechend umsichtig muss die passende Formel für den zu untersuchenden Fall ausgewählt werden, um die Realität möglichst genau wiederzugeben.\\
...
Aufgrund der Unkenntnis der Parameter \(P_{redeposition}\) und \(s\) kann auf diese Formel bei der tatsächlichen Berechnung an W7-X nicht zurückgegriffen werden. Stattdessen wird die sehr vereinfachende Annahme gemacht, dass jedes auftreffende Kohlenstoffion an der Targetoberfläche haften bleibt und eine chemisch identische Struktur zu der des Ursprungsmaterial mit atomarer Dichte \(n_{carbon}\) in (\unit[per-mode = symbol]{\per\m\tothe{3}}) aufbaut. Daher lautet die Formel zur Abschätzung der Dicke der Wiederablagerungsschicht \(\Delta_{deposition}\) in \unit{m} nach einer Entladung mit der Dauer \(t_{discharge}\) in \unit{s}

\begin{align}
	\Delta_{deposition} = \Gamma_C \cdot \frac{t_{discharge}}{n_{carbon}} \text{   .}
\end{align}

Analog zur Erosionsrate ist die Depositionsrate \(\frac{\Delta_{deposition}}{t_{discharge}}\) in \unit[per-mode=symbol]{\m\per\s}.
